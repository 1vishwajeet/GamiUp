-- Update the get_admin_custom_challenges function to get challenge type from creator's participant record
...
AS $function$
  SELECT 
    ...
    COALESCE(
      (SELECT cp.game_id FROM public.contest_participants cp WHERE cp.contest_id = c.id AND cp.user_id = c.created_by LIMIT 1), 
      'Custom'
    ) as challenge_type
