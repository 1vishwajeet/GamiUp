-- Update admin_delete_contest function to NEVER delete participant data
-- Only move contest to history and preserve all participant/payment data
...
BEGIN
  ...
  -- ALWAYS move contest to history and NEVER delete participant data
  ...
  -- ONLY delete the contest from main table (NEVER delete participants or payments)
  DELETE FROM public.contests WHERE id = p_contest_id;
  
  IF participant_count > 0 THEN
    RETURN json_build_object(
      'success', true, 
      'message', 'Contest deleted successfully. All ' || participant_count || ' participant records and payments have been preserved.'
    );
  ELSE
